...
public interface GameResultRepository extends JpaRepository<GameResult, UUID> {
    @Query("SELECT TOP (:matches) gr.game_result_id, gr.challenged_points, gr.challenger_points, g.* FROM game_result gr LEFT JOIN game g on gr.game_id = g.game_id WHERE g.challenger_id = :teamId OR g.challenged_id = :teamId ORDER BY g.final_date_time DESC")
