...
public interface GameResultRepository extends JpaRepository<GameResult, UUID> {
    ...
    @Query(nativeQuery = true, value = "SELECT gr.*, g.* FROM game_result gr JOIN game g ON gr.game_id = g.game_id WHERE gr.confirmed = 1 AND (g.challenger_id = :teamId OR g.challenged_id = :teamId)")
